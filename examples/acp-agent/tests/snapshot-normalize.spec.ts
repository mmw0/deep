...
describe('normalizeSessionLog', () => {
  ...
  it('leaves event data fields untouched beyond the time zeroing (no per-event field scrubs)', () => {
    const ev = JSON.stringify({
      type: 'hook/result', seq: 2, time: 5,
      data: { turn: 1, point: 'UserPromptSubmit', handlerId: 'h', decision: 'block', exitCode: 2 },
    })
    const out = normalizeSessionLog(`${header({})}\n${ev}\n`, ctx)
