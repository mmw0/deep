...
| `plan-acp-agent` | `pnpm run test:snapshot` — the session-mode wire surface plus the recorded plan-mode approve/keep-planning arcs, replayed keyless | re-record the two recorded scenarios (`test:snapshot:record`) |
