...
- `agent/prompt-submit(agent, content, source, signal, next) → PromptDecision` — waterfall, fired per drained queued message inside the open turn, before the `user/message` append. `signal` belongs to that turn and `next` remains the final parameter. `allow` optionally rewrites the prompt `content` or attaches separately sourced `additionalContexts[]`; `block` drops the prompt and the loop appends a durable `prompt/blocked` in its place (see the dispatch note below).
