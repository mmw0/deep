...
- `/merge` appends exactly one length-capped `context/message` to the parent with source `plugin: sidechat`; the parent's next request sees it, and `session/load` reproduces the visible handback note identically to the live stream — both derive from the same durable event.
