...
一波评审意见同时落在一条依赖堆叠（`A ← B ← C …`）中的多个 PR（Pull Request）上。本文规定在不破坏堆叠的前提下解决这些意见的纪律。它依赖的两个不变式是根 [AGENTS.md](../../AGENTS.md) § Conventions 中的常设指令：只用 merge commit，以及永远不改写已推送的分支。
