...
| Plan mode | 已交付：[`@deepseek-ai/dsh-mode`](../../packages/mode/mode/README.md) — 落日志的 `mode/set` 状态、`mode:policy` 引导段、沙箱 `access` 上限（`bash/resolve-mode` clamp + 上限派生的 bash 守卫），以及经用户评审的 `exit_plan_mode` 出口 |
