...
The **DeepSeek Harness SDK** is a Cordis-based SDK for building agent harnesses. The governing principle: **everything is a plugin**; the shipped agent loop is just one plugin in the default bundle, not a privileged kernel.
...
The default distribution is a composition, not a hierarchy. `packages/core/` is a repository grouping for the default agent spine; capability seams around it are equally first-class plugins to Cordis.
