...
The seam supports trusted-plugin `stdin` and ordinary `env`, but the model-facing tool exposes neither. It builds requests only from declared arguments, signal, owner, and the registry-collected `dshEnv`; extra model keys are ignored and cannot replace managed values. Shell syntax provides equivalent command-level behavior, while the local executor scrubs ambient credentials and stale `DSH_*` values. See the [stdin/env RFC](../../../docs/rfc/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-surface.md).
