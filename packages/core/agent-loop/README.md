    ...
    STEP loop:
      ...
      schedule tool-calls: group by tools.executionMode (exclusive call = barrier;
        ...
        each STARTED call: session('tool/call')  ⟵ model-order per started call; log positions
          → ordered tools/pre-execute/guards → pooled dispatch/body → ordered tools/post-execute → tools/result
            may interleave with sibling results as the pool replenishes
