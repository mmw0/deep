...
- `ctx.tools.executionMode(exec: ToolExecutionInput): ToolExecutionMode` Classify how one call may be scheduled relative to its step-siblings in the caller's visible scope — `{ kind: 'parallel' }` only when the resolved tool's `isConcurrencySafe(exec.arguments)` returns `true`, else `{ kind: 'exclusive' }` (unknown or restricted-away tool, no declaration, non-`true`, or a thrown check). The agent-loop scheduler uses it to group calls; host-only, never model-visible.
...
- `ToolDefinition` — `ToolSchema` + `execute(args, exec)`, optional presentation callbacks, cooperative `timeoutMs`, and optional `isConcurrencySafe(args)` concurrency metadata. `isConcurrencySafe` is host-only and never sent to the model.
...
- `ToolExecutionMode` — `{ kind: 'parallel' } | { kind: 'exclusive' }`, returned by `executionMode`. Object-tagged (not a bare boolean) so a future resource-grouping dimension can extend a variant without a breaking change.
