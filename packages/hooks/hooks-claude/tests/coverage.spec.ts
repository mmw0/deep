...
describe('hooks-claude coverage — config option arms + substitution + skip warning', () => {
  async function captureTranscriptPath(sessionRoot?: string): Promise<{ payload: { transcript_path: string }; expected: string | undefined }> {
    const d = dir()
  ...
  it('uses an empty transcript_path without a persistence locator', async () => {
    expect((await captureTranscriptPath()).payload.transcript_path).toBe('')
  })
