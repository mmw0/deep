...
import { nixePathOf, WorkspacePickFlow } from './WorkspacePicker.tsx'
...
> & {
  ...
  /** Open the browser-owned session delete-confirmation dialog (permanent wipe). */
...
export function WorkspaceBrowser({
  ...
  // Session delete is the permanent wipe: the confirmation dialog commits it
  // only after an explicit second click, mirroring the workspace delete flow.
  // The dialog stays pending until the removed id has left the list mirror.
  ...
  useEffect(() => {
    if (sessionDeleteTarget === null || sessionList.byId[sessionDeleteTarget.sessionId] !== undefined) return
  ...
  const confirmSessionDelete = () => {
    ...
    setSessionDeleteError(null)
    deleteSession(sessionDeleteTarget.sessionId).catch((reason: unknown) => {
      // The host frame may already have removed the row: only a real wire
      // failure (agent-busy, storage fault) keeps the dialog open.
      setSessionDeleting(false)
