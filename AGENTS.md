...
- **No hardcoded tunables in plugins**: a value two deployments could want set differently — a timeout, grace period, output cap, result limit, model name, base URL — is a defaulted, validated field on the plugin's `Config`, not a bare literal; a `DEFAULT_*` constant or test-only seam is not configurability (the test: changeable from `cordis.yml` without a code edit). Protocol/wire constants, external-spec values, and security invariants stay hardcoded (`dsh-web-fetch-local` is the exemplar).
