...
This is the monorepo of the DeepSeek Harness group; it hosts **DeepSeek Harness SDK**. The codebase is built on vendored Cordis, microkernel-style: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/` — the service map, event surface, loop lifecycle, and extension seams. The documentation standard is [docs/AGENTS.md](docs/AGENTS.md).
